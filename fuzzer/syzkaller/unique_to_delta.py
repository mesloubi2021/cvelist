...
```
The [COMPONENT] in the Linux kernel has a [PROBLEMTYPE] vulnerability. This vulnerability could be exploited by an attacker to cause memory corruption when [ROOTCAUSE] and cause [IMPACT] via [VECTOR].
```
...
ANSWER:""" % (json.dumps(list(crashes)), json.dumps(list(commits)), json.dumps(msgs))
    ...
    try:
        print(
            ...
            file=sys.stderr)
        return json.loads(response.content)["predictions"][0]['content'] + " This vulnerability exists in all versions of the Linux Kernel from %s until commit %s (%s)." % (
