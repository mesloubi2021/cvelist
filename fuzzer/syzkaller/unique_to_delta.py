...
def summarize(authed_session, commits, crashes, upstream_fix, fixed_tags, vuln_tags):
    ...
    for commit in upstream_fix:
        ...
        try:
            msgs.append(json.loads(response.content[5:])['message'][0:1000])
        except json.decoder.JSONDecodeError:
            pass
    prompt = """Provide a summary following the template:
```
[PROBLEMTYPE] in [COMPONENT] could be used to cause [IMPACT] via [VECTOR] because [ROOTCAUSE].
```
...
- VECTOR: What type of attackers used exploit this? (say if the vulnerability could be remote, local, physicial or unspecified)
...
ANSWER: Out of bounds vulnerability in the crypto subsystem of the Linux Kernel could be used to cause memory corruption because they read an uninitialized IV pointer.
...
ANSWER: Use-after-free vulnerability in the llc subsystem of the Linux Kernel could be used to cause memory corruption when passing a zero value for sllc_arphrd when LLC expected an ethernet device.
...
ANSWER: Use-after-free vulnerability in the can subsystem of the Linux Kernel could be used to cause memory corruption when the driver accessed data after having it freed.
...
ANSWER: Use-after-free vulnerability in the block subsystem of the Linux Kernel could be used to cause memory corruption because the bdi could be freed after inode_detach_wb when using certain cgroup configurations.
...
ANSWER: Use-after-free vulnerability in the qrtr subsystem of the Linux Kernel could be used to cause memory corruption when the socket was not properly detached from the qrtr otherwise skb->k may still reference this socket when released in tun->queue.
...
ANSWER:""" % (json.dumps(list(crashes)), json.dumps(list(commits)), json.dumps(msgs))
    ...
    try:
        print(
            ...
            file=sys.stderr)
        return json.loads(response.content)["predictions"][0]['content'] + " Since version %s until commit %s (%s)" % (
